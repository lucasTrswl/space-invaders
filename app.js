...
function moveShooter(e) {
    ...
    switch(e.key) {
        case 'ArrowLeft': // Si la flèche gauche est pressée
            ...
            e.preventDefault() // Perm et de ne pas bouger la fenêtre
...
alienInvaders.forEach(invader => {
    squares[invader].classList.add('alien');
  })
...
for(i = 0; i<alienInvaders.length; i++){
  ...
  if( squares[alienInvaders[i]].classList.contains('right')){
      if( fromLeft == true){
        direction = 20;
        fromLeft = false;
      }else{
  ...
  if( squares[alienInvaders[i]].classList.contains('left')){
    if( fromRight == true){
      direction = 20;
      fromRight = false;
    }else{
